...
def get_product_info(page_,url_,user=''):
    ...
    name_tag = soup.find_all(class_='B_NuCI')
    if name_tag:
        product_name = name_tag[0].get_text()
        # print(product_name)

    price_tag = soup.find_all(class_='_16Jk6d')
    if price_tag:
        price_th = price_tag[0].get_text()[1:]
        price = int(price_th.replace(',',''))
        # print(price)

    availability_tag = soup.find_all(class_='_16FRp0')
    availability = 'In stock'
    if availability_tag:
        availability = availability_tag[0].get_text()
    # print('availability:',availability)


    picture_tag = soup.find_all(class_='q6DClP')

    if picture_tag:
        picture_tag_soup = BeautifulSoup(str(picture_tag[0]),'html.parser')
